  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        ...
        <SectionTitle class="text-xs text-gray-600">Informações essenciais para edição de Carros no sistema.
        </SectionTitle>
      </div>
      ...
      <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
        ...
        <div>
          <span class="p-float-label">
            ...
            <label for="nome" class="text-sm">Marca</label>
          </span>
        </div>

        <div>
          <span class="p-float-label">
            <InputText v-model="form.cor" id="cor" type="text" class="w-full" required maxlength="50" />
            <label for="nome" class="text-sm">Cor</label>
          </span>
        </div>
        ...
        <div>
          <span class="p-float-label">
            ...
            <label for="nome" class="text-sm">Nome do Dono</label>
          </span>
        </div>
      ...
      <div class="flex space-x-5 mt-8">
        ...
        <Link :href="route('list.ConfigMotos')" as="button" type="button"
...
created_at: $propsPage?.value.ConfigMotos?.created_at,
...
function submit() {
  ...
  submitForm.post(
    route("update.ConfigMotos", { id: $propsPage.value?.ConfigMotos?.token }),
{
  preserveState: true,
    onError: (errors) => {
