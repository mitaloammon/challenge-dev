<template>
  ...
  <Nav
    class="mt-4 text-sm text-gray-400 list-none bg-white p-3 px-10 rounded-sm flex space-x-10 shadow-sm"
  >
    <button
      class="font-bold"
      :class="{ 'text-primary': currentNav == 1 }"
      @click="currentNav = 1"
    >
      Informações Gerais
    </button>
  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        <SectionTitle class="text-xs text-gray-600 font-bold uppercase"
          >INFORMAÇÕES GERAIS</SectionTitle
        >
        <SectionTitle class="text-xs text-gray-600"
          >Informações essenciais para edição de Carros no sistema.</SectionTitle
        >
      </div>
      ...
      <div class="flex space-x-5 mt-8">
        ...
        >
          <svg
            role="status"
            v-show="submited"
            class="mr-2 w-4 h-4 animate-spin fill-primary"
            viewBox="0 0 100 101"
            fill="none"
...
const form = useForm({
  ...
  created_at: $propsPage?.value.ConfigCarros?.created_at,
});
...
function submit() {
  ...
  submitForm.post(
    route("update.ConfigCarros", { id: $propsPage.value?.ConfigCarros?.token }),
    {
      preserveState: true,
      onError: (errors) => {
