  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        ...
        <SectionTitle class="text-xs text-gray-600">Informações essenciais para inserção de  Logs Erros no sistema.</SectionTitle>
      </div>
      ...
      <div class="flex space-x-5 mt-8">
        <button type="submit" :disabled="sending" class="p-2 flex rounded-mdbg-primary text-white px-6 text-sm font-medium items-center" :class="{'bg-opacity-80 cursor-not-allowed': submited}" v-if="$page.props.userPermissions.includes('edit.LogsErros')">
...
const form = useForm({
	pagina: $propsPage?.value.logsErros?.pagina,
modulo: $propsPage?.value.logsErros?.modulo,
erro: $propsPage?.value.logsErros?.erro,
erro_completo: $propsPage?.value.logsErros?.erro_completo,
created_at: $propsPage?.value.logsErros?.created_at,
...
function submit() {
  ...
  submitForm.post(
    route("update.logsErros", { id: $propsPage.value?.logsErros?.id }),
    {
      preserveState: true,
      onError: () => {
