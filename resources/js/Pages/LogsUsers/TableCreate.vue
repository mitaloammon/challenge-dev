<PageTitle title="Logs Users" />
	<Nav class="mt-4 text-sm text-gray-400 list-none bg-white p-3 px-10 rounded-sm flex space-x-10 shadow-sm " style="margin-bottom: 2%;">
		<div class="grid grid-cols-6 gap-6 max-md:grid-cols-1">	
		<div>
		<Link  as="button" :href="route('list.logsUsuario')" class="font-bold" :class="{'text-primary': currentNav == 1}" @click="currentNav = 1">
		 Logs Users
		</Link>
		</div>
		</div>
	...
	<div class="bg-white px-4 shadow-md rounded-xl">
	  ...
	  <div class="flex max-md:flex-reverse items-center">
		<button @click="toggleFilter = !toggleFilter" class="mt-4 p-3 shadow-md hover:bg-primary flex space-x-2 items-center cursor-pointer hover:text-white rounded-lg">
		...
		<button @click="resetFilter" class="text-gray-500 mt-4 ml-4 p-3 shadow-md flex space-x-2 items-center cursor-pointer rounded-lg" >
	  ...
	  <SlideUpDown v-model="toggleFilter" :duration="300">
		...
		<div class="text-sm text-gray-500 flex items-center">
		  ...
		  <div class="mt-10 grid grid-cols-8 gap-6 max-md:grid-cols-1">
...
<div class="flex items-center space-x-2">
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['id_ref']" />
				<span> ID REF</span>
			  	</div>
<div class="flex items-center space-x-2">
	  ...
	  <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
		<div class="inline-block min-w-full py-2 align-middle">
		  <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Logs Users List</b></h2>
...
<th style="text-align:center; font-size:11pt; border-width: 2.25pt 1pt 2.25pt 2.25pt;" class="cursor-pointer border-r group"  v-if="formColumns.columns.tipo" scope="col" @click="orderBy = {column: 'tipo', sorting: sortTable(sortVal.tipo) ? sortVal.tipo = 1 : sortVal.tipo = 0}"><b> TIPO</b></th>
<th style="text-align:center; font-size:11pt; border-width: 2.25pt 1pt 2.25pt 2.25pt;" class="cursor-pointer border-r group"  v-if="formColumns.columns.modulo" scope="col" @click="orderBy = {column: 'modulo', sorting: sortTable(sortVal.modulo) ? sortVal.modulo = 1 : sortVal.modulo = 0}"><b> MODULO</b></th>
<th style="text-align:center; font-size:11pt; border-width: 2.25pt 1pt 2.25pt 2.25pt;" class="cursor-pointer border-r group"  v-if="formColumns.columns.acao" scope="col" @click="orderBy = {column: 'acao', sorting: sortTable(sortVal.acao) ? sortVal.acao = 1 : sortVal.acao = 0}"><b> ACAO</b></th>
<th style="text-align:center; font-size:11pt; border-width: 2.25pt 1pt 2.25pt 2.25pt;" class="cursor-pointer border-r group"  v-if="formColumns.columns.id_ref" scope="col" @click="orderBy = {column: 'id_ref', sorting: sortTable(sortVal.id_ref) ? sortVal.id_ref = 1 : sortVal.id_ref = 0}"><b> ID REF</b></th>
<th style="text-align:center; font-size:11pt; border-width: 2.25pt 1pt 2.25pt 2.25pt;" class="cursor-pointer border-r group"  v-if="formColumns.columns.created_at" scope="col" @click="orderBy = {column: 'created_at', sorting: sortTable(sortVal.created_at) ? sortVal.created_at = 1 : sortVal.created_at = 0}"><b> CREATED AT</b></th>

				<th scope="col" style="text-align:center; border-width: 2.25pt 1pt 2.25pt 2.25pt;" class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6" ><svg xmlns="http://www.w3.org/2000/svg" style="text-align:center;" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
...
<td v-if="formColumns?.columns?.tipo" class="whitespace-nowrap py-2  text-sm"><input v-model="data.tipo" @change="updatelogsUsuario(key)" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.modulo" class="whitespace-nowrap py-2  text-sm"><input v-model="data.modulo" @change="updatelogsUsuario(key)" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.acao" class="whitespace-nowrap py-2  text-sm"><input v-model="data.acao" @change="updatelogsUsuario(key)" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.id_ref" class="whitespace-nowrap py-2  text-sm"><input v-model="data.id_ref" @change="updatelogsUsuario(key)" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.created_at" class="whitespace-nowrap py-2  text-sm"><input v-model="data.created_at" @change="updatelogsUsuario(key)" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>

				  <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6">
				  <button type="button" 
				  @click="setModalDeleteVal(data.id)" >
...
<td v-if="formColumns?.columns?.tipo" class="whitespace-nowrap py-4 text-sm"><input v-model="form.tipo" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.modulo" class="whitespace-nowrap py-4 text-sm"><input v-model="form.modulo" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.acao" class="whitespace-nowrap py-4 text-sm"><input v-model="form.acao" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.id_ref" class="whitespace-nowrap py-4 text-sm"><input v-model="form.id_ref" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>
<td v-if="formColumns?.columns?.created_at" class="whitespace-nowrap py-4 text-sm"><input v-model="form.created_at" type="text" class="w-full mt-1 rounded-md" maxlength="50" placeholder="" required /></td>			
				<td  class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6">
				<div>
				<span class="p-float-label">
					<button type="button" 
					@click="addlogsUsuario()">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5" style="color:green">
...
<Pagination :links="dataTable" :limit="recordValue" :searchBy="searchBy" :byStatus="statusValue"/>
	</div>
	<ModalDelete v-model:open="showDeleteModal.show" @del="DeletelogsUsuario(showDeleteModal.id)"/>
  </template>
  ...
  const InputsRequired = { tipo: "",modulo: "",acao: "",id_ref: "",created_at: "", };
...
function addlogsUsuario() {
    ...
    form.post(route('storeAjax.logsUsuario'), {
      preserveScroll: true,
  ...
  function DeletelogsUsuario(logsIndex) {
	const id = logsIndex;
    Inertia.post(route('deleteAjax.logsUsuario', {id: id}, ), {
      preserveScroll: true,
  ...
  function updatelogsUsuario(logsIndex) {
    const logs =  props.dataTable?.data[logsIndex]; 
    const id = logs.id;
    Inertia.post(route('updateAjax.logsUsuario', {id: id}, ), logs, {
      preserveScroll: true,
