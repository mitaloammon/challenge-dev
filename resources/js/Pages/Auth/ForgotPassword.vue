  ...
  <form @submit.prevent="submit" class="h-screen flex body-background">
    <div class="bg-white w-[24rem] m-auto p-10 rounded-lg shadow-md flex flex-col justify-center">
      ...
      <div class="mb-4 mt-6 flex justify-center">
        <span class="font-bold">Insira o e-mail de recuperação</span>
      </div>
      <div class="space-y-5">
        <span class="p-float-label">
          <InputText required type="text" v-model="form.email" />
          <label>E-mail *</label>
        </span>
        <span class="text-xs text-center flex justify-center bg-red-500 rounded-md p-3 text-white" v-if="$page.props.errorBags?.default">
          {{$page.props.errorBags?.default?.[0]?.[0] || $page.props.errorBags?.default?.email?.[0]}}
        </span>
        <div class="">
        ...
        <div class="flex justify-center">
          <Link :href="route('login')" class="text-proconph uppercase font-extrabold text-[12px]">Voltar para login</Link>
        </div>
        <div class="flex justify-center">
          <span class="text-gray-400 text-[10px]">Projeto Estagio ProConph - Todos os Direitos Reservados ©</span>
        </div>
      </div>
    </div>
  </form>
...
function submit() {
  sending.value = true;
  form.post(route("forgot.password.send"), {
    onSuccess: () => {
	  form.reset();
