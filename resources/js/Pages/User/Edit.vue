<template>
  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      ...
      <MultiSelect class="w-full" v-model="form.empresa" :options="Empresas" optionLabel="name" dataKey="value" required />
      <label for="empresa" class="text-sm">Empresa</label>
        ...
        <span class="p-float-label">
          <InputText id="username" type="email" v-model="form.email" :class="{'p-invalid': errorBags?.default?.email?.[0] }" />
      ...
      <div class="flex space-x-5 mt-8">
        <button type="submit" :disabled="sending" class="p-2 flex rounded-md bg-primary text-white px-6 text-sm font-medium items-center" :class="{'bg-opacity-80 cursor-not-allowed': submited}" v-if="$page.props.userPermissions.includes('edit.users')">
    ...
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 2">
      ...
      <Permission v-if="form.link_group == 'false'" :setChecked="getPermissionsArray()" v-model:checked="form.permission" />
      <div class="flex space-x-5 mt-8">
        <button type="submit" :disabled="sending" class="p-2 flex rounded-md bg-primary text-white px-6 text-sm font-medium items-center" :class="{'bg-opacity-80 cursor-not-allowed': submited}"  v-if="$page.props.userPermissions.includes('edit.users')">
          <svg role="status" v-show="submited" class="mr-2 w-4 h-4 animate-spin fill-primary" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"></path>
...
props.ExplodeEmpresa.split(",").forEach(function (data) {
  CamposFinal.push({ value: parseInt(data) });
    }); 

const form = useForm({
  empresa: CamposFinal,
  user_id: props.user1?.id,
  name: props.user1?.name,
  email: props.user1?.email,
  profile_picture: "",
  phone: props.user1?.phone,
  status: {value:props.user1?.status},
  link_group: props.user1?.link_group,
  group_permissions: props.user1?.roles?.at(-1)?.id,
  password: "",
...
function validateForm() {
  ...
  if ((form.link_group == true && form.group_permissions == false && form.permission == '')){
    throw toast.error(
      "Um grupo de permissões precisa ser preenchido."
    );
  }
}
...
function getPermissionsArray() {
  return props.user1?.permissions.map((val) => {
    return val?.id;
  });
}
...
function submit() {
  ...
  form.post(route("update.user", { id: props.user1?.id }), {
    ...
    onError: (errors) => {
      ...
      toast.error('Os Requisitos do Formulário não foram preenchidos.'); }
    },
    onSuccess: () => {
