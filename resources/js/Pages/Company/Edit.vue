<template>
  ...
  <button class="font-bold" :class="{'text-primary': currentNav == 1}" @click="currentNav = 1">
    ...
    <Link  as="button" :href="route('form.update.CompaniesCabecalho', {id: props.company?.id})" class="font-bold" v-if="$page.props.userPermissions.includes('list.OrdemServicoControle') || $page.props.userPermissions.includes('edit.OrdemServicoControle') || $page.props.userPermissions.includes('duplicate.OrdemServicoControle')">
    Cabeçalho Nota de Débito
    </Link>
</Nav>
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        ...
        <SectionTitle class="text-xs text-gray-600">Informações essenciais para edição de Empresas no sistema.</SectionTitle>
      </div>
...
const form = useForm({
  ...
  logo_path: "",
  quality_responsable: props.company?.quality_responsable,
  status: {value:props.company?.status},
  insc_municipal:  props.company?.insc_municipal,
  insc_estadual:  props.company?.insc_estadual,
  token_api:  props.company?.token_api,
  icms_situacao_tributaria: {codigo: props.company?.icms_situacao_tributaria},
  icms_origem: {codigo: props.company?.icms_origem}, 
  pis_situacao_tributaria: {codigo: props.company?.pis_situacao_tributaria}, 
  cofins_situacao_tributaria: {codigo: props.company?.cofins_situacao_tributaria}, 
  ...
  cidade:  props.company?.cidade,
  estado:  props.company?.estado,
});
...
function submit() {
  ...
  form.post(route("update.company", { id: props.company?.token }), {
    preserveState: true,
    onError: () => {
