  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        ...
        <SectionTitle class="text-xs text-gray-600">Informações essenciais para edição de Serviços no sistema.
        </SectionTitle>
      </div>
      ...
      <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
        ...
        <div>
          <span class="p-float-label">
            <InputText v-model="form.nome" id="descricao" type="text" class="w-full" required maxlength="50" />
        ...
        <div>
          <span class="p-float-label">
            <InputText v-model="form.preco" id="preco" type="text" class="w-full" required maxlength="50" />
        ...
        <div>
          <span class="p-float-label">
            <InputText v-model="form.tempo_estimado" id="tempo_estimado" type="text" class="w-full" required maxlength="50" />
        ...
        <div>
          <span class="p-float-label">
            <Dropdown class="w-full" v-model="form.status" :options="statusOption" optionLabel="name" dataKey="value"
              required />
      ...
      <div class="flex space-x-5 mt-8">
        ...
        <Link :href="route('list.ConfigServicos')" as="button" type="button"
...
const form = useForm({
  token: $propsPage?.value.ConfigServicos?.token,

  descricao: $propsPage?.value.ConfigServicos?.descricao,

  preco: $propsPage?.value.ConfigServicos?.preco,

  tempo_estimado: $propsPage?.value.ConfigServicos?.tempo_estimado,

  status: { value: $propsPage?.value.ConfigServicos?.status},

created_at: $propsPage?.value.ConfigServicos?.created_at,
...
function submit() {
  ...
  submitForm.post(
    route("update.ConfigServicos", { id: $propsPage.value?.ConfigServicos?.token }),
{
  preserveState: true,
    onError: (errors) => {
