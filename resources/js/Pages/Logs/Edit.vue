  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        ...
        <SectionTitle class="text-xs text-gray-600">Informações essenciais para inserção de  Logs no sistema.</SectionTitle>
      </div>
      ...
      <div class="flex space-x-5 mt-8">
        <button type="submit" :disabled="sending" class="p-2 flex rounded-mdbg-primary text-white px-6 text-sm font-medium items-center" :class="{'bg-opacity-80 cursor-not-allowed': submited}" v-if="$page.props.userPermissions.includes('edit.Logs')">
        ...
        <Link :href="route('list.logs')" as="button" type="button" class="p-2 rounded-md bg-secundary text-white px-6 text-sm font-medium">
        Voltar
        </Link>
      </div>
    </section>
...
const form = useForm({
  token: $propsPage?.value.logs?.token,
	
tipo: $propsPage?.value.logs?.tipo,

modulo: $propsPage?.value.logs?.modulo,

acao: $propsPage?.value.logs?.acao,

created_at: $propsPage?.value.logs?.created_at,
...
function submit() {
  ...
  submitForm.post(
    route("update.logs", { id: $propsPage.value?.logs?.token }),
    {
      preserveState: true,
      onError: () => {
