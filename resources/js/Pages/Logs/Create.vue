  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      <div class="flex flex-col space-y-1">
        ...
        <SectionTitle class="text-xs text-gray-600">Essential information for insertion Logs no sistema.</SectionTitle>
      </div>
...
<div>
  <span class="p-float-label">
   <InputText v-model="form.tipo" id="tipo" type="text" class="w-full" required />
...
<div>
  <span class="p-float-label">
   <InputText v-model="form.acao" id="acao" type="text" class="w-full" required />
    <label for="acao" class="text-sm">Ação</label>
    </span>
</div>
...
<div>
  <span class="p-float-label">
   <InputText v-model="form.created_at" id="created_at" type="text" class="w-full" required />
    <label for="created_at" class="text-sm"> </label>
    </span>
</div>
...
function submit() {
  ...
  submitForm.post(route("store.logs"), {
    preserveState: true,
    onError: () => {
