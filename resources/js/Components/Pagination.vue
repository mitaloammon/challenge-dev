      ...
      <div v-if="links?.links?.length > 2">
        <div class="flex flex-wrap -mb-1">
          <template v-for="(link, key) in links?.links">
            <div v-if="link.url === null" :key="key" class="mr-1 mb-1 px-4 py-3 text-xs leading-4 text-gray-400 border rounded-lg select-none cursor-not-allowed" v-html="link.label" />
            <Link preserve-scroll v-else :key="key + 1" class="mr-1 mb-1 px-4 py-3 text-xs leading-4 border rounded-lg" :class="{ 'text-whitebg-primary shadow-md': link.active }" :href="`${link.url}&limit=${props.limit}&searchBy=${props.searchBy}`" v-html="link.label" />
          </template>
        </div>
      </div>
    </div>
