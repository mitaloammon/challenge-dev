<template>
	<AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.SMTP')"  />
	<p><br></p>  
...
<div class="flex items-center space-x-2">
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['password']" />
				<span> Password</span>
			  	</div>
<div class="flex items-center space-x-2">
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['smtp']" />
				<span> Smtp</span>
			  	</div>
<div class="flex items-center space-x-2">
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['porta']" />
				<span> Porta</span>
			  	</div>
<div class="flex items-center space-x-2">
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['host']" />
				<span> Host</span>
			  	</div>
<div class="flex items-center space-x-2">
	  ...
	  <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
		<div class="inline-block min-w-full py-2 align-middle">
		  <h2 class="text-xl font-semibold fontInter text-center py-5"><b>SMTP</b></h2>
...
<th v-if="formColumns.columns.email" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'email', sorting: sortTable(sortVal.email) ? sortVal.email = 1 : sortVal.email = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800"> Email</span>
...
<th v-if="formColumns.columns.password" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'password', sorting: sortTable(sortVal.password) ? sortVal.password = 1 : sortVal.password = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800"> Password</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
			 	</th>
<th v-if="formColumns.columns.smtp" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'smtp', sorting: sortTable(sortVal.smtp) ? sortVal.smtp = 1 : sortVal.smtp = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800"> Smtp</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
			 	</th>
<th v-if="formColumns.columns.porta" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'porta', sorting: sortTable(sortVal.porta) ? sortVal.porta = 1 : sortVal.porta = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800"> Porta</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
			 	</th>
<th v-if="formColumns.columns.host" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'host', sorting: sortTable(sortVal.host) ? sortVal.host = 1 : sortVal.host = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800"> Host</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
...
<th v-if="formColumns.columns.created_at" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'created_at', sorting: sortTable(sortVal.created_at) ? sortVal.created_at = 1 : sortVal.created_at = 0}">
				<div class="flex">
				  ...
				  <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12">Ações</th>
				</tr>
			  </thead>
...
<td v-if="formColumns?.columns?.email" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
...
<td v-if="formColumns?.columns?.password" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.password }}</div>
					  </div>
					</div>
					 </td>
<td v-if="formColumns?.columns?.smtp" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.smtp }}</div>
					  </div>
					</div>
					 </td>
<td v-if="formColumns?.columns?.porta" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.porta }}</div>
					  </div>
					</div>
					 </td>
<td v-if="formColumns?.columns?.host" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.host }}</div>
					  </div>
					</div>
					 </td>
<td v-if="formColumns?.columns?.created_at" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
				  ...
				  <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6">
					<Actions :routeDel="route('delete.SMTP', {id: data?.token})" :routeUpdate="route('form.update.SMTP', {id: data?.token})" />
				  </td>
				</tr>
...
created_at: 1,
  ...
  const formColumns = useForm({
	route_of_list: "list.SMTP",
	columns: {
		
email: validateColumnsVisibility("email"),
password: validateColumnsVisibility("password"),
smtp: validateColumnsVisibility("smtp"),
porta: validateColumnsVisibility("porta"),
host: validateColumnsVisibility("host"),
