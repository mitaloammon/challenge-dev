...
<div class="bg-white px-4 shadow-md rounded-xl">
   <div class="flex max-md:flex-reverse items-center" >
	...
	<button @click="resetFilter" class="text-gray-500 mt-4 ml-4 p-3 shadow-md flex space-x-2 items-center cursor-pointer rounded-lg"  title="Limpar Filtros e Ordenações">
  	...
  	<Link as="button" :href="route('form.store.user')" class="bg-primary mt-4 ml-4 p-4 py-3 flex space-x-2 shadow-md text-white rounded-md text-sm hover:bg-indigo-900" v-if=" $page.props.userPermissions.includes('create.users')">
	  <span>+ Cadastro</span>
	</Link>
    ...
    <SlideUpDown v-model="toggleFilter" :duration="300">
      <div class="flex mb-4 max-md:flex-col-reverse items-center">
        <Limit class="p-field p-col-12 p-md-4 mt-6 max-md:w-full max-md:mb-5 flex items-center space-x-3 text-xs text-gray-500">
          <span>Mostrar</span>
          <select name="" @change="setParams" v-model="recordValue" id="" class="rounded-md focus:ring-proconph h-8 text-xs w-20">
      ...
      <div class="text-sm text-gray-500 flex items-center">
        <div class="space-x-6 flex ml-4  grid grid-cols-6 gap-6 max-md:grid-cols-1">
          ...
          <div class="flex items-center space-x-2">
            <Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['hierarchy']" />
            <span>Permissão</span>
          </div>
          <div class="flex items-center space-x-2">
    ...
    <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
      <div class="inline-block min-w-full py-2 align-middle">
        <h2 class="text-xl font-semibold fontInter text-center py-2"><b>Lista de Usuários</b></h2>
          ...
          <table class="min-w-full divide-y divide-gray-300">
            <thead class=" mb-24">
              <tr class="text-gray-500 font-bold select-none" @click="setParams">
                <th v-if="formColumns.columns.name" scope="col" class="py-3.5 px-4 text-left text-sm sm:pl-6 cursor-pointer flex items-center border-r group" @click="orderBy = {column: 'name', sorting: sortTable(sortVal.name) ? sortVal.name = 1 : sortVal.name = 0}">
                  <span class="group-hover:text-indigo-800">Nome / Email</span>
                ...
                <th v-if="formColumns.columns.hierarchy" scope="col" class="px-5 text-left text-sm border-r group">
                  <div class="flex">
                    <span>Hierarquia</span>
                  </div>
                </th>
                <th v-if="formColumns.columns.status" scope="col" class="px-4 text-sm cursor-pointer text-left border-r group" @click="orderBy = {column: 'status', sorting: sortTable(sortVal.status) ? sortVal.status = 1 : sortVal.status = 0}">
                  <div class="flex">
                ...
                <th v-if="formColumns.columns.created_at" scope="col" class="px-4 text-center text-sm cursor-pointer items-center border-r group" @click="orderBy = {column: 'created_at', sorting: sortTable(sortVal.created_at) ? sortVal.created_at = 1 : sortVal.created_at = 0}">
                  <div class="flex">
                ...
                <th scope="col" class="px-3 py-3.5 text-right text-sm sm:pr-12">Ações</th>
              </tr>
            </thead>
              ...
              <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20" :class="{'bg-gray-50': (key % 2)}">
                <td v-if="formColumns.columns.name" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="flex items-center">
                    <div class="h-10 w-10 flex-shrink-0" v-if="data?.profile_picture">
                      <img class="h-10 w-10 rounded-full" :src="data?.profile_picture" onerror="javascript: this.src = '/images/profile_default.png'" alt="" />
                    </div>
                    <div class="h-10 w-10 flex-shrink-0" v-if="!data?.profile_picture">
                      <img class="h-10 w-10 rounded-full" src="/images/profile_default.png" alt="" />
                    </div>
                    <div class="ml-4">
                ...
                <td v-if="formColumns.columns.hierarchy" class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 text-center">
                  
                  <span v-if="!data.customizada">Customizada</span>
                  <span v-if="data?.customizada">{{data?.customizada}}</span>
                </td>
                <td v-if="formColumns.columns.status" class="whitespace-nowrap px-3 py-4 text-sm text-gray-500  text-center">
                  <span class="inline-flex rounded-full bg-green-100 text-center px-4 text-xs font-semibold leading-5 text-green-800" v-if="data?.status">Ativo</span>
                  <span class="inline-flex rounded-full bg-red-100 text-center px-4 text-xs font-semibold leading-5 text-red-800" v-if="!data?.status">Inativo</span>
                </td>
                <td v-if="formColumns.columns.created_at" class="whitespace-nowrap px-3 text-center py-4 text-sm text-gray-500">{{ data?.data_final }}</td>
                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                  <Actions :routeDel="route('form.delete.user', {id: data?.id})" :routeUpdate="route('form.update.user', {id: data?.id})" />
                </td>
              </tr>
            </tbody>
          </table>
