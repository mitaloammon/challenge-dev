<template>
	<AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.companies')"  />
	<p><br></p>  
	...
	<Search class="p-field p-col-12 p-md-4 mt-6 max-md:w-full ml-auto">
	  ...
	  <SlideUpDown v-model="toggleFilter" :duration="300">
		...
		<div class="text-sm text-gray-500 flex items-center">
		  <span class="font-semibold">Visible Columns:</span>
		  <div class="space-x-6 flex ml-4">
		 	 
			<div class="flex items-center space-x-2">
			<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['name']" />
			<span>Name</span>
			</div>

			<div class="flex items-center space-x-2">
			<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['cnpj']" />
			<span>CNPJ</span>
			</div>
			<div class="flex items-center space-x-2">
			<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['quality_responsable']" />
			<span>Quality Responsable</span>
			</div>
			<div class="flex items-center space-x-2">
	  ...
	  <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
		<div class="inline-block min-w-full py-2 align-middle">
		  <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Clients List</b></h2>
		  <div class="overflow-hidden overflow-x-visible ring-1 ring-black ring-opacity-5 md:rounded-lg">
			<table class="min-w-full divide-y divide-gray-300">
			  <thead class=" mb-24">
				...
				<th v-if="formColumns.columns.name" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'name', sorting: sortTable(sortVal.name) ? sortVal.name = 1 : sortVal.name = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800">Name</span>
				...
				<th v-if="formColumns.columns.cnpj" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'cnpj', sorting: sortTable(sortVal.cnpj) ? sortVal.cnpj = 1 : sortVal.cnpj = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800">CNPJ</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
				...
				<th v-if="formColumns.columns.quality_responsable" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'quality_responsable', sorting: sortTable(sortVal.quality_responsable) ? sortVal.quality_responsable = 1 : sortVal.quality_responsable = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800">Quality Responsable</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
			 	</th>
				<th v-if="formColumns.columns.status" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'status', sorting: sortTable(sortVal.status) ? sortVal.status = 1 : sortVal.status = 0}">
				<div class="flex">
				...
				<th v-if="formColumns.columns.created_at" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'created_at', sorting: sortTable(sortVal.created_at) ? sortVal.created_at = 1 : sortVal.created_at = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800">Created At</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
				  ...
				  <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12">Accions</th>
				</tr>
			  </thead>
			  <tbody class="divide-y divide-gray-200 bg-white">
				...
				<td v-if="formColumns?.columns?.name" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					...
					<td v-if="formColumns?.columns?.cnpj" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.cnpj }}</div>
					  </div>
					</div>
					 </td>
					<td v-if="formColumns?.columns?.quality_responsable" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.quality_responsable }}</div>
					  </div>
					</div>
					 </td>
					<td v-if="formColumns?.columns?.status" class="whitespace-nowrap px-3 py-4 text-sm text-gray-500  text-center">
					<span class="inline-flex rounded-full bg-green-100 text-center px-4 text-xs font-semibold leading-5 text-green-800" v-if="data?.status">Active</span>
					<span class="inline-flex rounded-full bg-red-100 text-center px-4 text-xs font-semibold leading-5 text-red-800" v-if="!data?.status">InActive</span>
				  </td>
					<td v-if="formColumns?.columns?.created_at" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.created_at }}</div>
					  </div>
					</div>
					 </td>

				  <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6">
					<Actions :routeDel="route('delete.companies', {id: data?.token})" :routeUpdate="route('form.update.companies', {id: data?.token})" />
				  </td>
				</tr>
			  </tbody>
			</table>
...
name: 1,
cnpj: 1,
quality_responsable: 1,
status: 1,
created_at: 1,
  };
  
  const formColumns = useForm({
	route_of_list: "list.companies",
	columns: {
		
name: validateColumnsVisibility("name"),
logo_path: validateColumnsVisibility("logo_path"),
cnpj: validateColumnsVisibility("cnpj"),
quality_responsable: validateColumnsVisibility("quality_responsable"),
