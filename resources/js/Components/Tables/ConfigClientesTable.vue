<template>
	<div class="bg-white px-4 shadow-lg rounded-xl">
		<p><br></p>
		<AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigClientes')"
			:RotaExcel="route('get.Excel.ConfigClientes')" />
		...
		<SlideUpDown v-model="FiltroAvancado" :duration="300">
			<form @submit.prevent="submit">
				<div class="flex ml-4  grid grid-cols-8 gap-8 max-md:grid-cols-1">
                    ...
                    <div>
						<span class="p-float-label">
							<InputText v-model="form2.telefone" id="telefone" type="text" class="w-full" required />
                    ...
                    <div>
						<span class="p-float-label">
							<InputText v-model="form2.email" id="email" type="text" class="w-full" required />
                    ...
                    <div>
						<span class="p-float-label">
							<InputText v-model="form2.endereco" id="endereco" type="text" class="w-full" required />
							<label for="endereco" class="text-sm">Endereco</label>
						</span>
					</div>
		...
		<SlideUpDown v-model="toggleFilter" :duration="300">
			...
			<div class="text-sm text-gray-500 flex items-center">
				<div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
                    ...
                    <div class="flex items-center space-x-2">
						<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['telefone']" />
						<span>Telefone</span>
					</div>
					...
					<table class="min-w-full divide-y divide-gray-300">
						<thead class=" mb-24">
							<tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.telefone" scope="col"
									class="px-4 text-sm cursor-pointer text-center border-r group"
									@click="orderBy = { column: 'telefone', sorting: sortTable(sortVal.telefone) ? sortVal.telefone = 1 : sortVal.telefone = 0 }">
									<div class="flex">
										<span class="group-hover:text-indigo-800">Telefone</span>
										<svg xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
											viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
											<path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th v-if="formColumns.columns.email" scope="col"
									class="px-4 text-sm cursor-pointer text-center border-r group"
									@click="orderBy = { column: 'email', sorting: sortTable(sortVal.email) ? sortVal.email = 1 : sortVal.email = 0 }">
									<div class="flex">
										<span class="group-hover:text-indigo-800">Telefone</span>
										<svg xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                ...
                                <th v-if="formColumns.columns.endereco" scope="col"
									class="px-4 text-sm cursor-pointer text-center border-r group"
									@click="orderBy = { column: 'endereco', sorting: sortTable(sortVal.endereco) ? sortVal.endereco = 1 : sortVal.endereco = 0 }">
									<div class="flex">
										<span class="group-hover:text-indigo-800">Endereço</span>
										<svg xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
											viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
											<path stroke-linecap="round" stroke-linejoin="round"
								...
								<th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.ConfigClientes') || $page.props.userPermissions.includes('delete.ConfigClientes')">
									Ações</th>
							</tr>
						</thead>
						<tbody class="divide-y divide-gray-200 bg-white">
							<tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
								...
								<td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
									style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.ConfigClientes') || $page.props.userPermissions.includes('delete.ConfigClientes')">
									<Actions :routeDel="route('delete.ConfigClientes', { id: data?.token })"
										:routeUpdate="route('form.update.ConfigClientes', { id: data?.token })" />
								</td>
							</tr>
...
const formColumns = useForm({
	route_of_list: "list.ConfigClientes",
	columns: {
...
const form2 = useForm({
	...
	endereco: props.Filtros?.nome || null,
...
function del() {
	...
	selected.value = '';
	Inertia.post(route("deleteSelected.ConfigClientes", { id: valor.value }));
}
function delTodos() {
	Inertia.post(route("deletarTodos.ConfigClientes"));
}

function RestaurarTodos() {
	Inertia.post(route("RestaurarTodos.ConfigClientes"));
}
...
function setParams() {
 ...
 function FiltroAvancadoAplica() {
	...
	!orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";  
	submitForm.post(route("listP.ConfigClientes"), {
	  replace: false,
	  data,
	  onSuccess: () => {
