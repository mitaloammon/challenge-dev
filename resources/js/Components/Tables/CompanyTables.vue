<template>
  <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.company')"  />
	<p><br></p>  
   ...
   <div class="flex max-md:flex-reverse items-center" >
    ...
    <SlideUpDown v-model="toggleFilter" :duration="300">
      ...
      <div class="text-sm text-gray-500 flex items-center">
        <div class="space-x-6 flex ml-4  grid grid-cols-6 gap-6 max-md:grid-cols-1">
          ...
          <div class="flex items-center space-x-2">
            <Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['logo']" />
            <span>Logo</span>
          </div>        
          <div class="flex items-center space-x-2">
    ...
    <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
      <div class="inline-block min-w-full py-2 align-middle">
           <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Lista de Empresas</b></h2>
          ...
          <table class="min-w-full divide-y divide-gray-300">
            <thead class=" mb-24">
              <tr class="text-gray-500 font-bold select-none" @click="setParams">
                ...
                <th v-if="formColumns.columns.cnpj" scope="col" class="px-4 text-sm cursor-pointer text-left border-r group" @click="orderBy = {column: 'status', sorting: sortTable(sortVal.status) ? sortVal.status = 1 : sortVal.status = 0}">
                  <div class="flex">
                ...
                <th v-if="formColumns.columns.logo" scope="col" class="px-4 text-sm cursor-pointer text-left border-r group" @click="orderBy = {column: 'status', sorting: sortTable(sortVal.status) ? sortVal.status = 1 : sortVal.status = 0}">
                  <span class="group-hover:text-indigo-800">Logo</span>
                </th>
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20" :class="{'bg-gray-50': (key % 2)}">
                ...
                <td v-if="formColumns.columns.logo" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="flex items-center">
                    <img :src="data?.logo_path" class="w-8" alt="" srcset="">
                    <!-- <div class="font-medium text-gray-900">{{ data?.logo_path }}</div> -->
                  </div>
                </td>
                <td v-if="formColumns.columns.created_at" class="whitespace-nowrap px-3 text-center py-4 text-sm text-gray-500">{{ data?.created_at }}</td>
                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                  <Actions :routeDel="route('delete.company', {id: data?.id})" :routeUpdate="route('form.update.company', {id: data?.id})" />
                </td>
              </tr>
            </tbody>
          </table>
