<template>
	<div class="bg-white px-4 shadow-lg rounded-xl">
		<p><br></p>
		<AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigMotos')"
			:RotaExcel="route('get.Excel.ConfigMotos')" />
		...
		<SlideUpDown v-model="FiltroAvancado" :duration="300">
			<form @submit.prevent="submit">
				<div class="flex ml-4  grid grid-cols-8 gap-8 max-md:grid-cols-1">
					...
					<div>
						<span class="p-float-label">
							<InputText v-model="form2.marca" id="marca" type="text" class="w-full" required />
					...
					<div>
						<span class="p-float-label">
							<InputText v-model="form2.cor" id="cor" type="text" class="w-full" required />
					...
					<div>
						<span class="p-float-label">
							<InputText v-model="form2.anexo" id="anexo" type="text" class="w-full" required />
							<label for="anexo" class="text-sm">Anexo</label>
						</span>
					</div>
					...
					<div>
						<span class="p-float-label">
							<InputText v-model="form2.observacao" id="observacao" type="text" class="w-full" required />
		...
		<SlideUpDown v-model="toggleFilter" :duration="300">
			...
			<div class="text-sm text-gray-500 flex items-center">
				<div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
					...
					<div class="flex items-center space-x-2">
						<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['anexo']" />
						<span>Anexo</span>
					</div>
						...
						<thead class=" mb-24">
							<tr class="text-gray-500 font-bold select-none" @click="setParams">
								...
								<th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.ConfigMotos') || $page.props.userPermissions.includes('delete.ConfigMotos')">
									Ações</th>
							</tr>
						</thead>
						<tbody class="divide-y divide-gray-200 bg-white">
							<tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
								...
								<td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
									style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.ConfigMotos') || $page.props.userPermissions.includes('delete.ConfigMotos')">
									<Actions :routeDel="route('delete.ConfigMotos', { id: data?.token })"
										:routeUpdate="route('form.update.ConfigMotos', { id: data?.token })" />
								</td>
							</tr>
...
function del() {
	...
	selected.value = '';
	Inertia.post(route("deleteSelected.ConfigMotos", { id: valor.value }));
}
function delTodos() {
	Inertia.post(route("deletarTodos.ConfigMotos"));
}

function RestaurarTodos() {
	Inertia.post(route("RestaurarTodos.ConfigMotos"));
}
...
function setParams() {
 ...
 function FiltroAvancadoAplica() {
	...
	!orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";  
	submitForm.post(route("listP.ConfigMotos"), {
	  replace: false,
	  data,
	  onSuccess: () => {
