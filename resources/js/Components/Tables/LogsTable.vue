   ...
   <div class="flex max-md:flex-reverse items-center" >
    ...
    <SlideUpDown v-model="toggleFilter" :duration="300">
      ...
      <div class="text-sm text-gray-500 flex items-center">
        <div class="space-x-6 flex ml-4  grid grid-cols-6 gap-6 max-md:grid-cols-1">
            ...
            <Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['id_usuario']" />
            <span>Usuários</span>
          </div>
          <div class="flex items-center space-x-2">
    ...
    <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
      <div class="inline-block min-w-full py-2 align-middle">
        <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Lista de Logs</b></h2>
          ...
          <table class="min-w-full divide-y divide-gray-300">
            <thead class=" mb-24">
              <tr class="text-gray-500 font-bold select-none" @click="setParams">
				<th v-if="formColumns.columns.id_usuario" scope="col" class="px-4 text-sm cursor-pointer text-left border-r group" @click="orderBy = {column: 'id_usuario', sorting: sortTable(sortVal.id_usuario) ? sortVal.id_usuario = 1 : sortVal.id_usuario = 0}">
                  <div class="flex">
                    <span class="group-hover:text-indigo-800">Usuarios</span>
               ...
               <th v-if="formColumns.columns.tipo" scope="col" class="px-4 text-sm cursor-pointer text-left border-r group" @click="orderBy = {column: 'tipo', sorting: sortTable(sortVal.tipo) ? sortVal.tipo = 1 : sortVal.tipo = 0}">
                  <div class="flex">
                    <span class="group-hover:text-indigo-800">Tipo</span>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
                    </svg>
                  </div>
                 ...
                 <th v-if="formColumns.columns.acao" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'acao', sorting: sortTable(sortVal.acao) ? sortVal.acao = 1 : sortVal.acao = 0}">
                  <div class="flex">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20" :class="{'bg-gray-50': (key % 2)}">
 				...
 				<td v-if="formColumns?.columns?.id_usuario" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="flex items-center">
                    <div>
                      <div class="font-medium text-gray-900">{{ data?.usuario }}</div>
                    </div>
                  </div>
                </td>
                <td v-if="formColumns?.columns?.tipo" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="flex items-center">
                    <div>
                      <div class="font-medium text-gray-900">{{ data?.tipo }}</div>
                    </div>
                  </div>
                </td>
                ...
                <td v-if="formColumns?.columns?.location" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="flex items-center">
                    <div>
                      <div class="font-medium text-gray-900">{{ data?.location }}</div>
                    </div>
                  </div>
                </td>              
...
const formColumns = useForm({
  route_of_list: "list.logs",
  columns: {
	id_usuario: validateColumnsVisibility("id_usuario"),
