	...
	<Search class="p-field p-col-12 p-md-4 mt-6 max-md:w-full ml-auto">
	  ...
	  <SlideUpDown v-model="toggleFilter" :duration="300">
		...
		<div class="space-x-6 flex ml-4  grid grid-cols-6 gap-6 max-md:grid-cols-1">
		  ...
		  <div class="space-x-6 flex ml-4">
				...
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['pagina']" />
				<span>Página</span>
			  	</div>
				<div class="flex items-center space-x-2">
				...
				<div class="flex items-center space-x-2">
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['erro']" />
				<span>Erro</span>
			  	</div>
				<div class="flex items-center space-x-2">
				<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['erro_completo']" />
	  ...
	  <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
		<div class="inline-block min-w-full py-2 align-middle">
		  <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Listagem Logs Erros</b></h2>
...
<th v-if="formColumns.columns.pagina" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'pagina', sorting: sortTable(sortVal.pagina) ? sortVal.pagina = 1 : sortVal.pagina = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800">Página</span>
...
<th v-if="formColumns.columns.modulo" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'modulo', sorting: sortTable(sortVal.modulo) ? sortVal.modulo = 1 : sortVal.modulo = 0}">
				<div class="flex">
...
<th v-if="formColumns.columns.erro" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'erro', sorting: sortTable(sortVal.erro) ? sortVal.erro = 1 : sortVal.erro = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800">Erro</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
			 	</th>
<th v-if="formColumns.columns.erro_completo" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'erro_completo', sorting: sortTable(sortVal.erro_completo) ? sortVal.erro_completo = 1 : sortVal.erro_completo = 0}">
				<div class="flex">
				  <span class="group-hover:text-indigo-800">Erro Completo</span>
				  <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12" />
				  </svg>
				</div>
...
<td v-if="formColumns?.columns?.pagina" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900">{{ data?.pagina }}</div>
					  </div>
					</div>
					 </td>
...
<td v-if="formColumns?.columns?.erro" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900" style="max-width: 35ch; overflow: hidden;  text-overflow: ellipsis;  white-space: nowrap;" :title="data?.erro">{{ data?.erro }}</div>
					  </div>
					</div>
					 </td>
<td v-if="formColumns?.columns?.erro_completo" class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
					<div class="flex items-center">
					  <div>
						<div class="font-medium text-gray-900" style="max-width: 35ch; overflow: hidden;  text-overflow: ellipsis;  white-space: nowrap;" :title="data?.erro_completo">{{ data?.erro_completo }}</div>
					  </div>
					</div>
					 </td>
...
created_at: 1,
  ...
  const formColumns = useForm({
	route_of_list: "list.logsErros",
	columns: {
		
pagina: validateColumnsVisibility("pagina"),
modulo: validateColumnsVisibility("modulo"),
erro: validateColumnsVisibility("erro"),
erro_completo: validateColumnsVisibility("erro_completo"),
