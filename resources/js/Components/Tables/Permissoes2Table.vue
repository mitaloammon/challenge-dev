...
<div class="flex items-center space-x-2">
	  ...
	  <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
		<div class="inline-block min-w-full py-2 align-middle">
		  <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Permissões</b></h2>
...
<th v-if="formColumns.columns.name" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'name', sorting: sortTable(sortVal.name) ? sortVal.name = 1 : sortVal.name = 0}" style="width:70%">
				<div class="flex">
...
<th v-if="formColumns.columns.created_at" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'created_at', sorting: sortTable(sortVal.created_at) ? sortVal.created_at = 1 : sortVal.created_at = 0}">
				<div class="flex">
				  ...
				  <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width:5%">Ações</th>
				</tr>
			  </thead>
...
<td v-if="formColumns?.columns?.status" class="whitespace-nowrap px-3 py-4 text-sm text-gray-500  text-center">
					<span class="inline-flex rounded-full bg-green-100 text-center px-4 text-xs font-semibold leading-5 text-green-800" v-if="data?.status=='true'">Ativo</span>
					<span class="inline-flex rounded-full bg-red-100 text-center px-4 text-xs font-semibold leading-5 text-red-800" v-if="data?.status=='false'">Inativo</span>
				  </td>
...
created_at: 1,
  ...
  const formColumns = useForm({
	route_of_list: "list.Permissoes2",
	columns: {
