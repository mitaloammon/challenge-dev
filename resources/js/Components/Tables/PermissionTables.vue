...
<div class="flex items-center space-x-2">
	  ...
	  <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
		<div class="inline-block min-w-full py-2 align-middle">
		  <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Lista de Permissões</b></h2>
...
<th v-if="formColumns.columns.name" scope="col" class="px-4 text-sm cursor-pointer text-center border-r group" @click="orderBy = {column: 'name', sorting: sortTable(sortVal.name) ? sortVal.name = 1 : sortVal.name = 0}" style="width:90%">
				<div class="flex">
...
<td v-if="formColumns?.columns?.status" class="whitespace-nowrap px-3 py-4 text-sm text-gray-500  text-center">
				  ...
				  <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6">
					<Actions :routeDel="route('form.delete.permission', {id: data?.id})" :routeUpdate="route('form.update.permission', {id: data?.id})" />
				  </td>
				</tr>
...
<Pagination :links="dataTable" :searchBy="searchBy" :byStatus="statusValue"/>
	</div>
...
created_at: 1,
  ...
  const formColumns = useForm({
	route_of_list: "list.permission",
	columns: {
		
name: validateColumnsVisibility("name"),
guard_name: validateColumnsVisibility("guard_name"),
