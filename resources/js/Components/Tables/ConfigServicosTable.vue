<template>
	<div class="bg-white px-4 shadow-lg rounded-xl">
		<p><br></p>
		<AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigServicos')"
			:RotaExcel="route('get.Excel.ConfigServicos')" />
			...
			<div class="bg-white rounded-3xl shadow-2xl w-96 max-h-[250px]">
				<div class="flex flex-col items-center">
					<div class="flex space-x-6 mb-3 mt-2">
						<button @click="toggleFilter = !toggleFilter"
						...
						<button @click="FiltroAvancado = !FiltroAvancado"
		...
		<Toolbar>
			...
			<template #end>
				<SlideUpDown v-model="DeleteSelect" :duration="300">
					<button @click="openDelSelect = true"
				...
				<button @click="DeleteSelect = !DeleteSelect"
				...
				<button @click="openDelTodos = true"
				...
				<button @click="openRestaurarTodos = true"
				...
				<div class="flex ml-4  grid grid-cols-8 gap-8 max-md:grid-cols-1">
					...
					<div>
						<span class="p-float-label">
							<InputText v-model="form2.descricao" id="descricao" type="text" class="w-full" required />
                    ...
                    <div>
						<span class="p-float-label">
							<InputText v-model="form2.preco" id="preco" type="text" class="w-full" required />
                    ...
                    <div>
						<span class="p-float-label">
							<InputText v-model="form2.tempo_estimado" id="tempo_estimado" type="text" class="w-full" required />
							<label for="tempo_estimado" class="text-sm">Tempo estimado</label>
						</span>
					</div>
					...
					<div>
						<span class="p-float-label">
							<Dropdown class="w-full" v-model="form2.status" :options="statusOption" optionLabel="name"
					...
					<div>
						<span class="p-float-label">
							<Checkbox inputId="binary" v-model="form2.limparFiltros" :binary="true" style="margin:2%" />
							<label for="status" class="text-sm" style="margin-left: 5%;">Deseja Resetar o Filtro
								Aplicado?</label>
						</span>
					</div>
				</div>
		...
		<SlideUpDown v-model="toggleFilter" :duration="300">
			<div class="flex mb-4 max-md:flex-col-reverse items-center">
				<Limit
					...
					<span>Mostrar</span>
					<select name="" @change="setParams" v-model="recordValue" id=""
			...
			<div class="text-sm text-gray-500 flex items-center">
				<div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
					...
					<div class="flex items-center space-x-2">
						<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['descricao']" />
						<span>Descrição</span>
					</div>

                    <div class="flex items-center space-x-2">
						<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['preco']" />
						<span>Preço</span>
					</div>

                    <div class="flex items-center space-x-2">
						<Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['tempo_estimado']" />
					...
					<table class="min-w-full divide-y divide-gray-300">
						<thead class=" mb-24">
							<tr class="text-gray-500 font-bold select-none" @click="setParams">
								...
								<th v-if="formColumns.columns.descricao" scope="col"
									class="px-4 text-sm cursor-pointer text-center border-r group"
									@click="orderBy = { column: 'descricao', sorting: sortTable(sortVal.descricao) ? sortVal.descricao = 1 : sortVal.descricao = 0 }">
									<div class="flex">
                                ...
                                <th v-if="formColumns.columns.preco" scope="col"
									class="px-4 text-sm cursor-pointer text-center border-r group"
									@click="orderBy = { column: 'preco', sorting: sortTable(sortVal.preco) ? sortVal.preco = 1 : sortVal.preco = 0 }">
									<div class="flex">
										<span class="group-hover:text-indigo-800">Preço</span>
										<svg xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
											viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
											<path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th v-if="formColumns.columns.tempo_estimado" scope="col"
									class="px-4 text-sm cursor-pointer text-center border-r group"
									@click="orderBy = { column: 'tempo_estimado', sorting: sortTable(sortVal.tempo_estimado) ? sortVal.tempo_estimado = 1 : sortVal.tempo_estimado = 0 }">
									<div class="flex">
										<span class="group-hover:text-indigo-800">Tempo estimado</span>
										<svg xmlns="http://www.w3.org/2000/svg"
											class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
											viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
											<path stroke-linecap="round" stroke-linejoin="round"
								...
								<th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.ConfigServicos') || $page.props.userPermissions.includes('delete.ConfigServicos')">
									Ações</th>
							</tr>
						</thead>
						<tbody class="divide-y divide-gray-200 bg-white">
							<tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
								:class="{ 'bg-gray-50': (key % 2) }">
								<td class="whitespace-nowrap py-6 pl-4 pr-3 text-sm sm:pl-6">
									<div class="flex items-center">
										<div>
											<SlideUpDown v-model="DeleteSelect" :duration="300">
												<Checkbox inputId="id" name="selected" :value="data?.token"
													v-model="selected" />
											</SlideUpDown>
										</div>
									</div>
								...
								<td v-if="formColumns?.columns?.descricao"
                                ...
                                <td v-if="formColumns?.columns?.preco"
									class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
									<div class="flex items-center">
										<div>
											<div class="font-medium text-gray-900">{{ data?.preco }}</div>
										</div>
									</div>
								</td>

                                <td v-if="formColumns?.columns?.tempo_estimado"
									class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
									<div class="flex items-center">
										<div>
											<div class="font-medium text-gray-900">{{ data?.tempo_estimado }}</div>
										</div>
									</div>
								</td>
								...
								<td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
									style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.ConfigServicos') || $page.props.userPermissions.includes('delete.ConfigServicos')">
									<Actions :routeDel="route('delete.ConfigServicos', { id: data?.token })"
										:routeUpdate="route('form.update.ConfigServicos', { id: data?.token })" />
								</td>
							</tr>
...
const formColumns = useForm({
	route_of_list: "list.ConfigServicos",
	columns: {

		descricao: validateColumnsVisibility("descricao"),

        preco: validateColumnsVisibility("preco"),

        tempo_estimado: validateColumnsVisibility("tempo_estimado"),
...
const form2 = useForm({
	...
	descricao: props.Filtros?.descricao || null,

    preco: props.Filtros?.preco || null,

    tempo_estimado: props.Filtros?.tempo_estimado || null,
...
function del() {
	...
	selected.value = '';
	Inertia.post(route("deleteSelected.ConfigServicos", { id: valor.value }));
}
function delTodos() {
	Inertia.post(route("deletarTodos.ConfigServicos"));
}

function RestaurarTodos() {
	Inertia.post(route("RestaurarTodos.ConfigServicos"));
}
...
function setParams() {
 ...
 function FiltroAvancadoAplica() {
	...
	!orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";  
	submitForm.post(route("listP.ConfigServicos"), {
	  replace: false,
	  data,
	  onSuccess: () => {
		FiltroAvancado.value = false,
		form2.reset();
    },
	});
  }
  watch(
	...
	(novoValor, valorAntigo) => {
	  if (novoValor !== valorAntigo) {
		  ...
		  // alert(orderBy?.value);		
		const url = `${window.location.pathname}?page=1&limit=${recordValue.value}&orderBy[order]=${orderBy.value.column}&orderBy[sorting]=${orderBy.value.sorting}`;
