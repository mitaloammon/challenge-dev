  ...
  <form @submit.prevent="submit">
    <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
      ...
      <div class="flex space-x-5 mt-8">
        ...
        <Link :href="route('list.@@TROCARAQUI@@@')" as="button" type="button"
...
const form = useForm({
  token: $propsPage?.value.@@TROCARAQUI@@@?.token,

  nome: $propsPage?.value.@@TROCARAQUI@@@?.nome,

  

  status: { value: $propsPage?.value.@@TROCARAQUI@@@?.status},

created_at: $propsPage?.value.@@TROCARAQUI@@@?.created_at,
...
function submit() {
  ...
  submitForm.post(
    route("update.@@TROCARAQUI@@@", { id: $propsPage.value?.@@TROCARAQUI@@@?.token }),
{
  preserveState: true,
    onError: (errors) => {
