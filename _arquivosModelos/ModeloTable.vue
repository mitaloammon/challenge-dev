<template>
	<div class="bg-white px-4 shadow-lg rounded-xl">
		<p><br></p>
		<AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.@@TROCARAQUI@@@')"
			:RotaExcel="route('get.Excel.@@TROCARAQUI@@@')" />
						...
						<thead class=" mb-24">
							<tr class="text-gray-500 font-bold select-none" @click="setParams">
								...
								<th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.@@TROCARAQUI@@@') || $page.props.userPermissions.includes('delete.@@TROCARAQUI@@@')">
									Ações</th>
							</tr>
						</thead>
						<tbody class="divide-y divide-gray-200 bg-white">
							<tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
								...
								<td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
									style="width: 5%;"
									v-if="$page.props.userPermissions.includes('edit.@@TROCARAQUI@@@') || $page.props.userPermissions.includes('delete.@@TROCARAQUI@@@')">
									<Actions :routeDel="route('delete.@@TROCARAQUI@@@', { id: data?.token })"
										:routeUpdate="route('form.update.@@TROCARAQUI@@@', { id: data?.token })" />
								</td>
							</tr>
...
function del() {
	...
	selected.value = '';
	Inertia.post(route("deleteSelected.@@TROCARAQUI@@@", { id: valor.value }));
}
function delTodos() {
	Inertia.post(route("deletarTodos.@@TROCARAQUI@@@"));
}

function RestaurarTodos() {
	Inertia.post(route("RestaurarTodos.@@TROCARAQUI@@@"));
}
...
function setParams() {
 ...
 function FiltroAvancadoAplica() {
	...
	!orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";  
	submitForm.post(route("listP.@@TROCARAQUI@@@"), {
	  replace: false,
	  data,
	  onSuccess: () => {
